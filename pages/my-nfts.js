...
import {
  marketplaceAddress, nftmarketaddress
} from '../config'
...
export default function MyAssets() {
  ...
  if (loadingState === 'loaded' && !nfts.length) return (
        ...
        </Link> 
        {
          username && (
                <div className="md:ml-auto flex flex-wrap items-center text-base justify-center">
                  ...
                  <Link href="/my-nfts">
                  <a className="mr-7"> <p className="font-medium text-[#7366f0] duration-[300ms] transform-gpu subpixel-antialiased" style={{ fontFamily:'arial'}}>My Assets</p></a>
                  </Link>
    ...
    <div className="my-nfts-container flex justify-center min-h-screen">
      <h1 className="py-10 px-20 text-3xl text-white">No NFTs owned</h1>
    </div>
    </div>
  )
  return (
    ...
    <div className="my-nfts-container flex justify-center min-h-screen">
      <div className="p-4">
      <h1 className="px-10 py-5 text-xl text-center text-slate-400 ">Showing NFTs for address: </h1>
      <p className='px-4 py-2 text-center text-purple-400 '>{addr}</p>
              ...
              nfts.map((nft,i) =>(
                <div key={i} className='nft-container rounded-xl shadow overflow-hidden hover:scale-[1.02] duration-300 bg-[#2a2d50] w-fit h-fit mb-8'>
                  ...
                  <div className=''>
                    ...
                    <button className='w-full bg-[#7366f0] text-white font-bold py-2 px-12 hover:bg-white hover:text-[#9fa4dd] duration-150' onClick={() => listNFT(nft)}>Sell</button>
                  </div>
                </div>
            ))
          }
